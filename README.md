...
from dipps import (predict_emotion_from_edf_single, detect_device_model,
                   _select_channels_from_edf, bins_to_waves, _pad_or_truncate,
                   reduce_eeg_noise, _CHANNEL_CACHE, _MAX_CACHE_SIZE)
...
if os.path.exists(scaler_path):
    ...
    scaler = None
    print(f"  WARNING: scaler not found — using raw FFT values")
...
def _run_inference(temp_path):
    """
    Core inference — returns (emotion_label, avg_proba, classes).
    Used by both web and API endpoints.
    """
...
def upload_file():
    ...
    try:
        emotion, _, _ = _run_inference(temp_path)
        os.unlink(temp_path)
...
def predict_api():
    """
    JSON endpoint for Flutter app.
    Accepts multipart/form-data with 'file' field (EDF).
    Returns JSON with emotion, confidence, and probabilities.
    """
    start = time.time()
